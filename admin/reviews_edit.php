...
if( isset( $_POST['headline'] ) )
{
  ...
  if( $_POST['headline'] and $_POST['text'] and $_POST['game_id'] and $_POST['rating'] and is_numeric($_POST['game_id']) and is_numeric($_POST['rating']) and $_POST['rating'] >= 1 and $_POST['rating'] <= 10 )
  {
    ...
    $query = 'UPDATE reviews SET 
        game_id = "'.mysqli_real_escape_string( $connect, $_POST['game_id'] ).'", 
    ...
    set_message( 'Review has been updated' );
...
$query = 'SELECT * FROM reviews WHERE review_id = '.$id.' LIMIT 1';
...
<form method="post">
  ...
  <select name="game_id" id="game_id" required>
    ...
    <?php while( $game = mysqli_fetch_assoc( $games_result ) ): ?>
      <option value="<?php echo $game['game_id']; ?>" <?php echo ($game['game_id'] == $record['game_id']) ? 'selected' : ''; ?>>
        <?php echo htmlentities( $game['title'] ); ?>
      </option>
    <?php endwhile; ?>
  </select>
  ...
  <input type="text" name="headline" id="headline" value="<?php echo htmlentities( $record['headline'] ); ?>" required>
  
  <br>
  
  <label for="text">Review Text:</label>
  <textarea name="text" id="text" rows="10" required><?php echo htmlentities( $record['text'] ); ?></textarea>
  ...
  <select name="rating" id="rating" required>
    ...
    <?php for($i = 1; $i <= 10; $i++): ?>
      <option value="<?php echo $i; ?>" <?php echo ($i == $record['rating']) ? 'selected' : ''; ?>><?php echo $i; ?>/10</option>
    <?php endfor; ?>
  </select>
